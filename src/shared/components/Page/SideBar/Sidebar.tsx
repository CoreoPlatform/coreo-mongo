        ...
        fetch("/api/collections")
            ...
            .then((data) => {
                if (Array.isArray(data.collections)) {
                    setCollections(
                        (data.collections as CollectionMetrcis[]) || [],
                    );
                }
                setLoading(false);
            })
        ...
        <aside className="w-64 md:max-h-[calc(100vh-40px)] sticky top-0 bg-gradient-to-b from-green-900 to-green-800 dark:bg-gray-200 border-r p-4 overflow-y-auto">
            <Link href={"/"}>
                <button className="flex justify-start items-start w-full focus:outline-none focus:text-indigo-400 text-gray-200 text-left py-5">
                    <p className="text-lg leading-5 font-roboto font-medium uppercase">
                        Home
                    </p>
                </button>
            </Link>
            <button
                onClick={() => setDropDownCollections(!dropDownCollections)}
                className="focus:outline-none focus:text-indigo-400 text-left  text-gray-200 flex justify-between items-center w-full py-5 space-x-14"
            >
                <p className="text-lg leading-5 font-roboto font-medium uppercase">
                    {databaseName}
                </p>
            ...
            {!dropDownCollections && (
                <div className="flex justify-start flex-col items-start pb-5">
                    {collections.length === 0 ? (
                        <p className="text-base text-gray-100 leading-4">
                            No one collection
                        </p>
                    ) : (
                        collections.map((c) => (
